    ...
    <section id="gallery" className="mt-10 scroll-mt-28">
      <div className="flex flex-wrap items-center gap-3">
        <h2 className="font-display text-lg font-extrabold text-ink-900">
          {T("원본 사진 모음", "Photo Collection", "Зургийн цуглуулга")} <span className="text-[12px] font-semibold text-ink-400">· {photos.length}{T("점", "", "")}</span>
        </h2>
        {hasColor && (
      ...
      <p className="mt-1 text-[12px] text-ink-400">{T("공개 자료에서 본인으로 확인된 사진입니다. 기본은 원본(흑백)이며, ‘컬러’를 누르면 같은 톤의 AI 복원본으로 봅니다.", "Photos identified as this person from public sources. The default is the original (black-and-white); press 'Color' for an AI-restored version in the same tone.", "Нийтийн эх сурвалжаас тухайн хүн гэж тогтоосон зургууд. Үндсэн нь эх хувь (хар цагаан); 'Өнгөт' дарвал ижил өнгөтэйгөөр AI сэргээсэн хувилбарыг харна.")}</p>
          ...
          return (
            <figure key={i} className="mb-4 break-inside-avoid overflow-hidden rounded-2xl border border-ink-200 bg-white">
              <span className="relative block">
                ...
                {colored && (
                  <span className="absolute left-1.5 top-1.5 rounded-full px-1.5 py-0.5 text-[8.5px] font-extrabold" style={{ background: "rgba(40,26,14,.78)", color: "#ffe7c2" }}>{T("AI 복원", "AI restored", "AI сэргээсэн")}</span>
                )}
              </span>
